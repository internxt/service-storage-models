 ...
 * @private
 */
Marketing.statics._verifyReferralLink = async function(referralLink, callback) {
  const Marketing = this;

  try {
    const marketing = await Marketing.findOne({ referralLink });
 ...
 */
Marketing.statics.create = async function(email, callback) {
  const Marketing = this;

  try {
    const marketing = await Marketing.findOne({ user: email });
