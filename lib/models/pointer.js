...
Pointer.methods._validate = function() {
  ...
  assert(
    Buffer.from(storj.utils.ripemd160sha256(this.hash), 'hex').length === 20,
 ...
 */
Pointer.statics.create = async function(shard, callback) {
  let Pointer = this;
  let pointer = new Pointer(shard);

  try {
    pointer._validate();
    const savedPointer = await pointer.save();
    callback(null, savedPointer);
  } catch (err) {
    callback(err);
  }
};
