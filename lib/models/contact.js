 ...
 */
ContactSchema.statics.record = async function(contactInfo, callback) {
  let Contact = this;
  ...
  try {
    const updatedContact = await Contact.findOneAndUpdate({ _id: contactInfo.nodeID }, contact, {
        upsert: true,
        new: true
      });
    return done(null, updatedContact);
  } catch (err) {
    return done(err);
  }
};
 ...
 * @param {Function} callback
 */
ContactSchema.statics.recall = async function(num, callback) {
  try {
    const result = await this.find({}).limit(num).sort({ lastSeen: -1 });
...
ContactSchema.statics.updateLastContractSent = async function(id, callback) {
  try {
    const result = await this.updateOne({ _id: id}, { $set: { lastContractSent: Date.now() } });
