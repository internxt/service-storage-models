 ...
 */
PublicKey.statics.create = async function(user, pubkey, label, callback) {
  ...
  try {
    const existingPubkey = await PublicKey.findOne({ _id: pubkey });
    if (existingPubkey) {
...
PublicKey.statics.validate = function(pubkey) {
  ...
  pubkey = Buffer.from(pubkey, 'hex').toString('hex');
