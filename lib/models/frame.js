 ...
 */
Frame.methods.lock = async function(callback) {
  this.locked = true;
 ...
 * @param {Function} callback
 */
Frame.methods.unlock = async function(callback) {
  this.locked = false;
...
Frame.methods.addShard = function(pointer, callback) {
  ...
  function maybePullShard(next) {
    ...
    if (replaced) {
      ...
      self.updateOne(pullUpdate, next);
    } else {
      next();
    }
  }
 ...
 * @param {Function} callback
 */
Frame.statics.create = async function(user, callback) {
  let Frame = this;
  let frame = new Frame({ user: user._id });

  try {
    const savedFrame = await frame.save();
    callback(null, savedFrame);
  } catch (err) {
    callback(err);
  }
};
