...
ShardSchema.statics.create = async function(item, callback) {
  let Shard = this;

  try {
    let shard = await Shard.findOne({ hash: item.hash });
    ...
    const savedShard = await shard.save();
    callback(null, { ...shard.toObject(), _id: savedShard._id });
  } catch (err) {
    callback(err);
  }
};
